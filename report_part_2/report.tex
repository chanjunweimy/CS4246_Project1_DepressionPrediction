...
\begin{document}
	...
	Thus, our aim here is to measure depression via voice recordings that is obtained from real-world dataset i.e. a human test-subject. \\	
	...
	\subsection{Planning and Decision Making Process} \label{pdm}
	...
	We will be using only two live test subjects as a precursor to our project, subject A, a victim of depression and subject B, an average human.
	To begin, we have generated a sample application or agent that is able to ask questions and record the subject's reply.
	This agent is set to ask a fixed number of non-biased questions so as to not influence the subject's state of mind and to reduce the variation between subjects.
	The reply here, however is of low importance as our primary objective is to measure the voice and cross-reference it to our model. \\

	The above is the procedure we use to collect data, fed into the GP and finally the agent will make a decision to whether the subject is depressed or not depressed.
	If the outcome results in a one with low confidence interval, the subject undergoes the same procedure after a week as passed.
