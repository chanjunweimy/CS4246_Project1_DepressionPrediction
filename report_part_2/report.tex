...
\begin{document}
	...
	Moreover, our process allows prioritizing of patient's appointments by their PHQ-8 score(representing severity of depression).
	Hence, we would need to use 2 gaussian processes(GP): Gaussian Process Classifier (GPC) and the Gaussian Process Regressor (GPR). 
	...
	We note that the process is a general framework and that different clinics have different operating processes, clinics should tweak the framework to better fit their needs. Hence, we would omit some detail in our model and leave them to the implementer.
	...
	Several example questions are given in the section titled `Proof of Concept'.
	...
	PHQ-8 score determines the levels of depression(from $0$-$24$, $0$ means no depression and $24$ means very depressed) of the patient, allowing the staff to piroritize appointments based on the PHQ-8 score.
	We would piroritize patients who are more depressed(predicted by GPR with a higher PHQ-8 score) will have higher pirority and vice versa.
	Through the appointment(s) with the psychiatrist, the ground truth of the depression and PHQ-8 labels will be obtained and can be used to train both GPC and GPR if required(if this datapoint was predicted with no confidence).
	As more data points are observed and subsequently used for training, both GPC and GPR 's accuraccy would be improved incrementally.

	\subsection{Insights to our model}
	\subsubsection{Gaussian Processes :}
	In our modelling, we used 2 GPs for regression and classification tasks respectively. 
	...
	Hence, the questions asked are irrelevant. The questions will be presented in the section titled `proof of concept'.
	...
	As mentioned in the modelling, we define a particular probability $\rho_c$. If GPC predicts with $p \geq \rho_c$, then we can trust the predicted label and continue to decide appropriately based on the label. 
	If GPC predicts with $p < \rho_c$, then we cannot trust the predicted label and refer the patient to an appointment with the psychiatrist.
	We determine the probability $\rho_c$ experimentally by observing the prediction quality of the labels in the training data, a summary is given in Table \ref{tab:rho}. We observe that with a $\rho_c = 70\%$, it would predict the depression label with $100\%$ accuraccy. Henceforth, in this paper we will define $\rho_c = 70\%$.
	...
	We have seen that this method dispenses limited resources with efficiacy\cite{rosedale2011effectiveness} throughout many emergency services world-wide. 
	...
	We also prioritize the clinic's resources based on a person's depression severity, allowing better allocation of resources. This ensures that the clinic's resources are directed to people who need them most.
