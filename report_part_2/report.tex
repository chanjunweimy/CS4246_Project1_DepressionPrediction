...
\begin{document}
	...
	\begin{figure}[h]
 	...
 	\begin{comment}
	@startuml
		Patient -right-|>(Phone Interview) : set appointment
		(Phone Interview) -right-|> [GP Classifier] : voice recording
		[GP Classifier] ..> (Continue observing) : <75\% CI
		[GP Classifier] --> (Is patient depressed?) : >=75\% CI
		(Is patient depressed?) --> [GP PHQ-8] : yes
		(Is patient depressed?) ..> [Refer to general practitioner] : no
		[GP PHQ-8] .left.|> (Determine priority)
	@enduml
 	\end{comment}
