...
\begin{document}
	...
	digraph g {
		graph [rankdir=LR];
	 	gp_class[shape=rect label="GP for Classifying"];
		gp_phq[shape=rect label="GP for PHQ-8"];
		patient[shape=circle];
		vp[shape=square label="Voice Recording"];
		pc[shape=square label="Phone Interview"];
		result_na[shape=square label="Continue observation"];
		result_ac[shape=square label="Is patient depressed?"];
		not_depressed[shape=square label="Refer to general practitioner"];
		depressed[shape=square label="Determine priority"];
		patient -> pc;
		pc -> vp;
		vp -> gp_class;
		gp_class -> result_na[label=<75\%];
		gp_class -> result_ac[label=>75\%];
		result_ac -> not_depressed;
		result_ac -> depressed;
		depressed -> gp_phq;
	}
	@enduml
	...
	This leads to two scenarios depending on the confidence interval:
	...
	This process also automates the selection process for psychiatrist and thus reduce bias in treatment.
