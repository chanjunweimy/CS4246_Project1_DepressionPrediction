...
class gaussClassify(classifier):
    ...
    def __init__(self, X=None, Y=None, equal=0, diagonal=0, wts=None, reg=0):
        ...
        ----------
        X : M x N numpy array 
            M = number data points; N = number of features.
        Y : M x 1 array 
        ...
        wts : M x 1 numpy array of positive weights (ints or floats).
        reg : int or float 
            Regularizes the covariance model.

        Properties:
          classes:  list of class identifiers
          probs  :  list of class probabilities for each class
          means  :  list of numpy arrays (1xN); mean of each class distribution
          covars :  list of numpy arrays (NxN); covariances of each class distribution
        """
        self.means = []
        self.covars = []
        self.probs = []
...
## CORE METHODS ################################################################
    ...
    def train(self, X, Y, equal=0, diagonal=0, wts=None, reg=0):
        ...
        if equal:                                               # force covariances to be equal (take weighted average)
            Cov = sum( [self.probs[i]*self.covars[i]] )
            for i,c in enumerate(self.classes):
              self.covars[i] = Cov
...
## HELPERS #####################################################################
    ...
    def __eval_gaussian(self, X, mean, covar):
        """
        This is a helper method that calculates the probability of X under a Gaussian distribution.
        """
        m,d = X.shape
        p = np.zeros((m, 1))                                    # store evaluated probabilities for each datum
        # normalizing constant for Gaussian
