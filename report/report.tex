...
\begin{document}
	...
	When a person is depressed, no matter the time of the day, we should also be able to determine the person is depressed according 
	to the speech signal. And we assume that the speech signals extracted from different depressed people should share some similarities and thus suitable 
	for prediction with the Gaussian Process Model.

	\subsection{Mathematical Axioms Used}
	...
	For a kernel to be used in Gaussian Process, one kernel must be  positive semidefinite:\\
	...
	\begin{enumerate}
		...
		\begin{equation}\label{eq:kernel_noise}
			\text{if} x == x', k(x,x') = \delta ; \text{else 0}
		\end{equation}
		\\
		...
		\item \textbf{Sum}\\
		Sum-kernel is the sum of two kernels k1 and k2.\\
	\end{enumerate}
