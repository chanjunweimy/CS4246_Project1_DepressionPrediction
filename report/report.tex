...
\begin{document}
	...
	GP provides prediction with a clear probabilistic interpretation with an estimate of prediction uncertainty. 
	From the prediction uncertainty, we are able to intelligently add data to improve the GP model. 
	Moreover, we are able to get a sense of the quality of each prediction from the prediction uncertainty. These properties are inherent from a GP model and are lacking in many other machine learning
	models. In our context, a person would be able to obtain both the predicted PHQ-8 score and also prediction confidence when he uses a GP model. Hence, he would be able to get a
	sense of the accuracy of the PHQ-8 score and take the appropriate action with greater confidence.\\
	
	 In addition, there exists algorithms for GP hyperparameter 	learning—something the other Machine Learning methods like SVM framework lacks. It is very useful as 
	 hyperparameter estimation could provide 
	...
	\begin{enumerate}
		\item \textbf{Audio Feature Subsets}	\\
		An audio signature can be uniquely defined by the following feature subsets:
		\begin{enumerate}
			\item \textbf{Energy}	\\
	...
	We used standard signal processing techniques to extract the 4 audio feature subsets (Energy, MFCC, Magnitude Spectrum, Zero-crossing) as presented in the Modelling and Approach section.
	Each audio feature subset comprises of several individual features and the breakdown of the actual number of feature columns are as follows:
