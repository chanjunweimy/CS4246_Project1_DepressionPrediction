...
\begin{document}
	...
	\caption{PHQ-8 scores' histogram of the PHQ-8 scores}
	...
	Henceforth, we would need to pre-process the auditory data before we use it in our Gaussian Process Model. However, the code to extract the data will be provided along with the report.
	...
	Since the focus of this paper is the prediction of the PHQ-8 score, we will not describe in detail the pre-processing of the audio files.
	We used standard signal processing techniques to extract the 4 audio features (Energy, MFCC, Magnitude Spectrum, Zero-crossing) presented in section [REF].
	Each audio features is composed of several individual features.

 	\begin{table}[h]
 		\begin{center}
  			\begin{tabular}{ | r | c | }
    			\hline
			 Audio Feature & Number of features \\ \hline
			 Magnitude Spectrum               & 512 \\ \hline
			 Mel Frequency Cepstral Coefficients           & 12 \\ \hline
			 Energy        & 1 \\ \hline
			 Zero-Crossing Rate & 1 \\ \hline \hline
			 Total & 526 \\ \hline
			 \end{tabular}
		\end{center}
 	\caption{Number of features in processed data}
 	\label{no_features}
 	\end{table}
	...
	We compared the proposed Gaussian Models (ARD RBF, Matern, and dot-product) against commonly used machine learning algorithms.
